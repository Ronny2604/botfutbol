...
# --- 2. INICIALIZAÇÃO DE ESTADOS (BLINDAGEM TOTAL ANTI-CRASH) ---
estado_padrao = {
...
# --- 5. CSS SUPREMO (O LAYOUT ESTÁVEL QUE FUNCIONA BEM) ---
css_stable = f"""
<style>
...
/* ABAS NATIVAS REESTILIZADAS SEGURAS (PÍLULA NO TOPO) */
...
# --- NAVEGAÇÃO PRINCIPAL (ABAS NO TOPO, SEGURAS) ---
        ...
        st.markdown(f"""
            <div class='glass-card' style='display:flex; justify-content:space-between; align-items:center; border-left: 4px solid #ff3333;'>
                <div>
                    ...
                    <span style='color:white; font-weight:bold; font-size: 14px; margin-left: 10px;'>{j_live['home_team']} {(minuto//15)%3} x {(minuto//25)%2} {j_live['away_team']}</span>
                </div>
...
with t2:
    ...
    if st.button("EXECUTAR SCANNER", use_container_width=True):
        with st.spinner("Procurando assimetrias na API..."):
            ...
            if not dados:
                ...
                st.warning("⚠️ Limite da API esgotado ou sem jogos hoje. A exibir simulação. Atualize a chave no Hub.")
            else:
            ...
            for jogo in dados[:10]:
                ...
                # --- A LÓGICA QUE APLICA O MERCADO ESCOLHIDO ---
                ...
                else: m = random.choice([f"Vitória {c}", "Ambas Marcam: Sim", "Over 1.5 Gols", "Over 8.5 Cantos"]) # Misto
    ...
    with st.expander("✍️ OVERRIDE MANUAL (Sua Própria Grade)"):
        st.markdown("<p style='font-size:11px; color:#888;'>Cole os jogos abaixo para a IA analisar o risco e sugerir a aposta:</p>", unsafe_allow_html=True)
...
with t3:
    ...
    if st.session_state.bilhete:
        ...
        for b in st.session_state.bilhete:
            odd_f *= b['o']
            st.markdown(f"<p style='margin:0; font-size:14px; border-bottom: 1px solid rgba(255,255,255,0.1); padding: 5px 0;'>✅ <b>{b['jogo']}</b> <span style='float:right; color:{cor_neon}; font-weight:bold;'>@{b['o']}</span><br><span style='font-size:11px; color:#aaa;'>Mercado: {b['m']}</span></p>".replace('\n', ''), unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if st.session_state.analises_salvas:
        for i, a in enumerate(st.session_state.analises_salvas):
            ...
            st.markdown(html_track.replace('\n', ''), unsafe_allow_html=True)
