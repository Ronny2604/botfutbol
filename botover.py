        ...
        for idx, item in enumerate(st.session_state.analisados):
            if item['conf'] >= min_conf:
                ...
                <div class='glass-panel' style='border-left: 5px solid {cor_neon}; padding: 15px;'>
                    ...
                    <div style='width: 100%; background-color: rgba(0,0,0,0.5); border-radius: 5px; margin-bottom: 8px; overflow: hidden;'>
                        <div style='width: {item['conf']}%; height: 6px; background-color: {cor_neon}; border-radius: 5px; box-shadow: 0 0 10px {cor_neon};'></div>
                    </div>
                    
                    <div style='display: flex; justify-content: space-between; font-size: 11px; color:#bbb; margin-bottom: 12px;'>
                        <div style='width: 48%;'>⚔️ ATK: {atk} 
                            <div style='width:100%; background:rgba(0,0,0,0.5); border-radius:3px; margin-top:2px;'>
                                <div style='width:{atk}%; height:4px; background:#ff3333; border-radius:3px;'></div>
                            </div>
                        </div>
                        <div style='width: 48%;'>🛡️ DEF: {dfs} 
                            <div style='width:100%; background:rgba(0,0,0,0.5); border-radius:3px; margin-top:2px;'>
                                <div style='width:{dfs}%; height:4px; background:#00e5ff; border-radius:3px;'></div>
                            </div>
                        </div>
                    </div>
...
# --- RECURSO PREMIUM NOVO: ABA BILHETE SAFE ---
with t3:
    ...
    # Lógica de "Semente Diária" para gerar sempre os mesmos 2 jogos por dia e trocar à meia-noite
    ...
    # Sorteia 2 jogos usando a semente fixa do dia
    safe_pick = random.sample(jogos_seguros_base, 2)
    random.setstate(estado_aleatorio_atual) # Restaura a aleatoriedade normal para o resto do app
