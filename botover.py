...
# --- 2. BLINDAGEM DE MEMÓRIA (O FIM DO ATTRIBUTE ERROR) ---
# Usamos .setdefault para garantir que as chaves existem, e acessamos via .get()
defaults = {
...
# --- 3. CSS (FIM DA TELA PRETA LATERAL) ---
cor_neon = "#00ff88"
c_prim = "#ffffff"
c_sec = "#8b9bb4"
...
st.markdown(f"""
    ...
    /* FIX NATIVO: Fundo aplicado diretamente ao app do Streamlit, sem larguras forçadas */
    .stApp {{
    ...
    /* Espaço extra no final para o menu bottom não tapar os botões */
    .block-container {{ padding-top: 1rem !important; padding-bottom: 110px !important; max-width: 600px !important; margin: 0 auto !important; }}
    ...
    /* MENU INFERIOR (BOTTOM NAV) - FIXADO COM SEGURANÇA 100% WIDTH */
    div[data-testid="stTabs"] > div:first-of-type {{ 
        ...
        padding: 10px 0 25px 0 !important; /* Padding bottom para a barra do iPhone */
    ...
    /* BOTÕES DA GRELHA */
    .stButton>button[kind="secondary"] {{ height: 80px !important; background-color: {bg_card} !important; border: 1px solid {border_card} !important; border-radius: 12px !important; color: {c_sec} !important; font-size: 12px !important; font-weight: 600 !important; transition: all 0.2s !important; }}
    ...
    /* ESTILOS DE CARTÃO E PROGRESSO */
    .card {{ background: {bg_card}; border: 1px solid {border_card}; border-radius: 12px; padding: 15px; margin-bottom: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); }}
...
if not st.session_state.get('autenticado'):
    ...
    nome_in = st.text_input("Credencial de Acesso:", placeholder="Insira o seu nome")
...
st.markdown(f"""
    <div style='position: sticky; top: 0; z-index: 999; background: rgba(11,16,26,0.9); backdrop-filter: blur(15px); padding: 15px; margin:-15px -15px 20px -15px; border-bottom: 1px solid {border_card};'>
        <div style='display:flex; justify-content:space-between; align-items:center;'>
            <div style='display:flex; align-items:center;'>
                ...
                <div>
                    ...
                    <div style='color:{c_sec}; font-size:10px;'>Status: <span style='color:{cor_neon};'>Online</span></div>
                </div>
            </div>
            <div style='text-align:right;'>
                <div style='color:{c_sec}; font-size:10px; text-transform:uppercase;'>Portfólio</div>
                <div style='color:{c_prim}; font-weight:900; font-size:18px;'>{fmt_moeda(saldo_atual)}</div>
            </div>
        </div>
        <div style='width:100%; height:2px; background:{border_card}; border-radius:2px; margin-top:10px;'><div style='width:{pct_banca}%; height:2px; background:{cor_banca}; box-shadow: 0 0 8px {cor_banca};'></div></div>
    </div>
""", unsafe_allow_html=True)
...
with t1:
    ...
    st.markdown(f"<h4 style='font-size:14px; color:{c_prim}; margin-top:10px;'>🏆 Ranking VIP Diário</h4>", unsafe_allow_html=True)
    st.markdown(f"""
...
# ==========================================
with t2:
    st.markdown(f"<p style='text-align:center; font-size:14px; font-weight:800; color:{c_prim};'>Filtro de Mercado</p>", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='text-align:center; color:{cor_neon}; font-size:11px; font-weight:bold; margin-bottom:15px;'>Foco Atual: {st.session_state.get('tipo_analise_selecionado')}</p>", unsafe_allow_html=True)
    ...
    codigo_da_liga = LIGAS[st.selectbox("Selecionar Liga:", list(LIGAS.keys()))]
    ...
    if st.button("ANALISAR AGORA", type="primary", use_container_width=True):
        with st.spinner("Conectando à API..."):
            ...
            if dados == "QUOTA_ERROR": 
                ...
                st.error("🚨 Limite da API esgotado. Mostrando dados Mock."); dados = gerar_dados_mock()
            elif dados == "AUTH_ERROR": 
                st.session_state['status_api'] = "SIMULAÇÃO"
                st.error("🚨 Chave Inválida. Mostrando dados Mock."); dados = gerar_dados_mock()
            elif not dados: 
                st.session_state['status_api'] = "SIMULAÇÃO"
                st.warning("⚠️ Sem jogos hoje. Mostrando dados Mock."); dados = gerar_dados_mock()
            else: 
                st.session_state['status_api'] = "REAL DATA"
                st.success("✅ Extração real bem-sucedida.")
            ...
            for jogo in dados[:5]:
                ...
                st.session_state['analisados'].append({
                    ...
                    "vol": random.randint(50, 1500), "ev": (odd > 1.7 and atk > 75)
                })

    with st.expander("✍️ Inserir Jogos Manuais"):
        grade = text = st.text_area("Formato: Time A x Time B")
        if st.button("Forçar Análise", use_container_width=True):
            if grade:
                st.session_state['analisados'] = []
                for j in [x for x in grade.split('\n') if 'x' in x.lower()]:
                    c, f = j.lower().split('x')[0].strip().title(), j.lower().split('x')[1].strip().title()
                    st.session_state['analisados'].append({"jogo": f"{c} x {f}", "m": "Aposta IA", "o": 1.85, "atk": 80, "def": 60, "conf": 92, "xg_c": 1.8, "xg_f": 0.9, "vol": 100, "ev": True})
    ...
    if st.session_state.get('analisados'):
        ...
        for idx, i in enumerate(st.session_state.get('analisados')):
            ...
            # String HTML achatada e blindada para st.write
            html = f"<div class='card'><div style='display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;'><span class='badge'>{i['m']}</span><span style='background:#05080f; padding:4px 8px; border-radius:6px; color:{cor_neon}; font-weight:900; border:1px solid {cor_neon}40;'>@{i['o']:.2f}</span></div><div style='color:{c_prim}; font-size:14px; font-weight:800; margin-bottom:5px;'>⚽ {i['jogo']} {ev_tag}</div><div style='display:flex; justify-content:space-between; color:{c_sec}; font-size:10px; margin-bottom:15px;'><span>Hoje</span><span>🐋 Vol: ${i['vol']}k</span><span>🎯 Conf: {i['conf']}%</span></div><div style='display:flex; justify-content:space-between; font-size:9px; color:{c_sec};'><span>Poder Ofensivo (xG {i['xg_c']})</span><span>{i['atk']}%</span></div><div class='progress-bg'><div class='progress-fill-atk' style='width:{i['atk']}%;'></div></div><div style='display:flex; justify-content:space-between; font-size:9px; color:{c_sec}; margin-top:4px;'><span>Muralha Defensiva (xG {i['xg_f']})</span><span>{i['def']}%</span></div><div class='progress-bg' style='margin-bottom:10px;'><div class='progress-fill-def' style='width:{i['def']}%;'></div></div><div style='background:rgba(0,0,0,0.3); border-left:2px solid {cor_neon}; padding:8px; border-radius:6px; font-size:10px; color:{c_sec}; font-style:italic;'>🤖 <b>IA Whisper:</b> Tendência forte para a equipa da casa no 2º tempo.</div></div>"
            ...
            with col2:
                if st.button(f"💾 SALVAR", key=f"s_{idx}", use_container_width=True): 
                    st.session_state['analises_salvas'].append(i); st.toast("Salvo!")
...
with t3:
    ...
    if bilhete:
        ...
        for b in bilhete:
            ...
            txt_bot += f"⚽ {b['jogo']}\n👉 {b['m']} (@{b['o']})\n\n"
        html_b += "</div>"
        st.write(html_b, unsafe_allow_html=True)
        ...
        st.session_state['usar_kelly'] = st.checkbox("🧠 Usar Critério de Kelly", value=st.session_state.get('usar_kelly', False))
        
        if st.session_state.get('usar_kelly'):
            prob = 1 / odd_f * 1.15
            kelly = max(0.01, min(((odd_f - 1) * prob - (1 - prob)) / (odd_f - 1), 0.05))
            rec_stake = banca_disp * kelly
        ...
        st.markdown(f"<div style='font-size:10px; color:{c_sec}; text-align:center; margin-bottom:15px;'>🛡️ <b>Hedge:</b> Aposte {fmt_moeda(valor*0.3)} no 'Empate' na Betfair para proteger.</div>", unsafe_allow_html=True)
        
        txt_bot += f"📊 ODD TOTAL: @{odd_f:.2f}\n💰 GESTÃO: {fmt_moeda(valor)}"
        st.markdown(f'<a href="https://api.whatsapp.com/send?text={urllib.parse.quote(txt_bot)}" target="_blank" style="display:block; text-align:center; background:rgba(37,211,102,0.2); color:#25d366; padding:12px; border-radius:8px; font-weight:bold; text-decoration:none; margin-bottom:15px; border:1px solid #25d366;">📲 COPIAR PARA TELEGRAM/WPP</a>', unsafe_allow_html=True)
    ...
    st.markdown(f"<h3 style='color:{c_prim}; font-size:16px; margin-top:30px;'>📂 Tracking Salvo</h3>", unsafe_allow_html=True)
    salvas = st.session_state.get('analises_salvas', [])
    if salvas:
        for i, a in enumerate(salvas):
            st.write(f"<div class='card' style='border-left:3px solid #00d2ff; padding:10px; margin-bottom:5px;'><div style='font-size:12px; font-weight:bold; color:{c_prim};'>{a['m']} <span style='color:{cor_neon};'>@{a['o']}</span></div><div style='font-size:10px; color:{c_sec};'>{a['jogo']}</div></div>", unsafe_allow_html=True)
...
with t4:
    ...
    with st.expander("🔑 Chave API (The Odds)"):
        st.write(f"<span style='font-size:11px; color:{c_sec};'>Para jogos reais ilimitados, insira sua chave the-odds-api.com:</span>", unsafe_allow_html=True)
    ...
    with st.expander("🏛️ Bancas e UI"):
        st.session_state['bancas']["Betano"] = st.number_input("Banca Atual", value=st.session_state.get('bancas')["Betano"], step=50.0)
    
    with st.expander("📡 Console de Auditoria (Logs)"):
        st.write(f"<div style='background:#05080f; padding:10px; border-radius:8px; font-family:monospace; color:{cor_neon}; font-size:10px;'> > AUTH KEY: {st.session_state.get('api_key_odds')[:5]}...<br>> LAST PING: {datetime.now().strftime('%H:%M:%S')}<br>> FETCHING SYNDICATE DATA...<br>> STATUS: {st.session_state.get('status_api')}</div>", unsafe_allow_html=True)
