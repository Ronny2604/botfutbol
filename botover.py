...
# --- 2. INICIALIZAÇÃO "NUCLEAR" DE MEMÓRIA (FIM DO ATTRIBUTE ERROR) ---
if 'init_done' not in st.session_state:
    st.session_state.update({
        ...
        'historico_greens': [], 'status_api': "AGUARDANDO VARREDURA",
        'init_done': True
    })
...
# --- 3. CSS ROOT (FIM DAS BORDAS PRETAS) ---
cor_neon = "#00ff88"
st.write(f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;800;900&display=swap');
    
    /* ATAQUE DIRETO AO BODY - PREENCHIMENTO 100% GARANTIDO */
    body {{
    ...
    /* TORNA O STREAMLIT INVISÍVEL PARA MOSTRAR O FUNDO DO BODY */
    ...
    /* INPUTS */
    div[data-baseweb="input"] > div, div[data-baseweb="select"] > div, textarea {{ background-color: rgba(18,24,36,0.9) !important; border: 1px solid #1c2436 !important; color: white !important; border-radius: 8px !important; }}
    ...
    div[data-testid="stTabs"] > div:first-of-type {{ 
        ...
        background: rgba(18,24,36,0.95) !important; backdrop-filter: blur(25px) !important;
        border-radius: 30px !important; border: 1px solid {cor_neon}50 !important;
        padding: 5px 10px !important; z-index: 99999 !important; box-shadow: 0 10px 40px rgba(0,0,0,0.9) !important;
...
if not st.session_state.get('autenticado', False):
    ...
    st.write(f"<h1 style='text-align:center; font-weight:900; font-size:48px; color:white;'>V8 <span style='color:{cor_neon}; text-shadow: 0 0 20px {cor_neon};'>A.I.</span></h1>", unsafe_allow_html=True)
    st.write(f"<p style='text-align:center; color:#8b9bb4; font-size:12px; margin-bottom:40px; text-transform:uppercase;'>Terminal Institucional</p>", unsafe_allow_html=True)
...
with t1:
    ...
    st.write(f"<h4 style='font-size:14px; color:white; margin-top:20px;'>🏆 Ranking Global VIP</h4>", unsafe_allow_html=True)
    st.write(f"""
...
# ==========================================
with t2:
    st.write(f"<p style='text-align:center; font-size:14px; font-weight:800; color:white;'>Direcionamento Algorítmico</p>", unsafe_allow_html=True)
    ...
    st.write(f"<p style='text-align:center; color:{cor_neon}; font-size:11px; font-weight:bold; margin-bottom:15px;'>Foco Selecionado: {st.session_state.get('tipo_analise_selecionado')}</p>", unsafe_allow_html=True)
    ...
    if st.button("INICIAR VARREDURA", type="primary", use_container_width=True):
        with st.spinner("Conectando à The Odds API..."):
            ...
            for jogo in dados[:5]:
                ...
                m = f"Vitória {c}"
                tipo = st.session_state.get('tipo_analise_selecionado', '')
                if "Gols" in tipo: m = "Over 2.5 Gols"
                elif "Cantos" in tipo: m = "Over 8.5 Cantos"
                elif "Ambas" in tipo: m = "Ambas Marcam: Sim"
    ...
    # RENDERIZAÇÃO BLINDADA (ST.WRITE EM UMA LINHA)
    if st.session_state.get('analisados'):
        ...
        for idx, i in enumerate(st.session_state.get('analisados')):
            ...
            # String HTML pura sem quebras de linha para evitar o bug do Streamlit Markdown
...
with t3:
    ...
    if bilhete:
        ...
        hedge_val = valor * 0.3
        st.write(f"<div style='font-size:10px; color:#8b9bb4; text-align:center; margin-bottom:15px;'>🛡️ <b>Hedge:</b> Aposte {fmt_moeda(hedge_val)} no 'Empate' para proteger capital.</div>", unsafe_allow_html=True)
    ...
    st.write(f"<h3 style='color:white; font-size:16px; margin-top:30px;'>📂 Tracking (Validação)</h3>", unsafe_allow_html=True)
    salvas = st.session_state.get('analises_salvas', [])
    if salvas:
        for i, a in enumerate(salvas):
            st.write(f"<div style='background:rgba(18,24,36,0.85); padding:10px; border-left:3px solid #00d2ff; margin-bottom:5px; border-radius:4px;'><div style='font-size:12px; font-weight:bold; color:white;'>{a['m']} <span style='color:{cor_neon};'>@{a['o']}</span></div><div style='font-size:10px; color:#8b9bb4;'>{a['jogo']}</div></div>", unsafe_allow_html=True)
...
# ==========================================
with t4:
    st.write(f"<h3 style='color:white; font-size:18px; margin-bottom:20px;'>⚙️ HUB VIP</h3>", unsafe_allow_html=True)
    ...
    with st.expander("🏛️ Bancas e UI"):
        st.selectbox("Tema do App:", ["🟢 Verde Hacker"], key="tema_escolhido")
    ...
    with st.expander("📡 Console de Auditoria (Deep Web Logs)"):
        st.write(f"<div style='background:#000; padding:10px; border-radius:8px; font-family:monospace; color:{cor_neon}; font-size:10px;'> > AUTH KEY: {st.session_state.get('api_key_odds')[:5]}...<br>> LAST PING: {datetime.now().strftime('%H:%M:%S')}<br>> FETCHING SYNDICATE DATA...<br>> STATUS: {st.session_state.get('status_api')}</div>", unsafe_allow_html=True)
